...
(function ($, window, document, undefined) {
	...
	_default.settings = {
		...
		showButton: true,   // 添加，默认鼠标移上去显示增删改的按钮
	...
	*/
	...
	Tree.prototype._setInitialState = function (node, level, done) {
		...
		$.each(node.nodes, $.proxy(function (index, node) {
			...
			node.nodeId = (parent && parent.nodeId) ?
											...
											(level - 1) + '.' + node.index;
			node.id =  Number(node.id);    // 添加：展示分类的id，便于增删改
	...
	Tree.prototype._template = {
		...
		// 添加：自定义的增删改按钮
		button: {
			add: $('<button onclick="addMenu(this)" class="btn btn-primary glyphicon glyphicon-plus node-hidden pull-right"</button>'),
			edit: $('<button onclick="editMenu(this)" class="btn btn-info glyphicon glyphicon-edit node-hidden pull-right"></button>'),
			remove: $('<button onclick="delMenu(this)" class="btn btn-danger glyphicon glyphicon-trash node-hidden pull-right"></button>')
		}
...
// 获取指定节点下的所有子节点ID
var listChildrenIds = function(parentNode) {
